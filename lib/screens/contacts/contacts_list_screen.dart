...
class _ContactsListScreenState extends State<ContactsListScreen> {
  ...
  void _startDeletionProcess() async {
    ...
    // ).show(context);
     TopMessageService().showMessage(
        context: context,
        message: 'Deleted $_deletionSuccessCount contacts${_deletionErrorCount > 0 ? '. $_deletionErrorCount failed' : ''}',
  ...
  Future<void> _addMultipleContactsToGroup(BuildContext context, String groupName, String groupPeriod, int groupFrequency, List<Contact> contacts, ThemeProvider themeProvider) async {
    ...
    TopMessageService().showMessage(
        context: context,
        message: 'Added $_addingSuccessCount contacts to $groupName${_addingErrorCount > 0 ? '. $_addingErrorCount failed' : ''}',
        backgroundColor: Colors.green,
        icon: Icons.check,
      );
